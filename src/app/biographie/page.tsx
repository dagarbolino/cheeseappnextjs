...
async function getData(): Promise<CheesePageData> {
  const res = await fetch(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.BIO_PAGE}`, {
        ...
        <div className="max-w-4xl mx-auto space-y-16">
          {data.body.map((section, index) => (
            ...
            >
              ...
              <div className="prose prose-lg max-w-none dark:prose-invert prose-headings:font-serif prose-headings:text-cheese-900 dark:prose-headings:text-cheese-500 prose-strong:text-cheese-700 dark:prose-strong:text-cheese-400">
                {section.value.content?.map((content) => (
                  <div
                    key={content.id}
                    className="text-gray-700 dark:text-gray-300 leading-relaxed [&>p:first-child]:text-lg [&>p:first-child]:font-medium [&>p:first-child]:text-cheese-800 dark:[&>p:first-child]:text-cheese-500"
